  ...
  return (
    <div className="mt-5 flex flex-col relative">
      <label className="relative">
        ...
        >
          {showDropdown
            ? `Selected workset [${TagsContainer.length}/${roleList.length}]`
            : "Select Workset"}
        </span>
      ...
      {showDropdown && (
        ...
        >
          ...
          {Array.isArray(filteredRoleList) && filteredRoleList.length !== 0 ? (
            filteredRoleList.map((workset) => {
              return (
                <CheckBoxWorksets
                  key={workset.id}
                  isChecked={TagsContainer.some(
                    (roleWorksets) => roleWorksets.name === workset.name
                  )}
                  role={workset}
            ...
            <p className="pl-4 px-3 py-2">No workset available</p>
          )}
        </div>
      )}
    </div>
...
export default EditWorksetsDropdown;
